...
const ViewServiceMessagesModel = ({
  ...
  getPayload: function (p_options) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      let sql = 'SELECT DISTINCT scnMessages.EQID as eqid, Equation.LRUID as lruid, ATAChapter.ChapterDesc as ata_name, ' +
        ...
        ' FaultCode as faultCode, Equation.EqDesc as details, ata, dateTime as date_time, fligthLegCounter, fligthLeg, fligthPhase, status, occurences, Equation.`EqDesc` as description, scenarioID, scnMessages.id as id ' +
        ' FROM scnMessages ' +
      ...
      self.getPayload(p_options).then(function (result) {
        ...
        self.getATAPayload().then(function (result2) {
          ...
          Promise.all(promisePayload).then(function(){
            ...
            self.getMessagesPayload().then(function (result3) {
              for (let j in result3) {
                formatted.edit.messages.push({
...
module.exports = ViewServiceMessagesModel
