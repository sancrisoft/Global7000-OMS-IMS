<template>
    <div class="scnFaultMessages">
      <form class="scnEditForm no-margin" @submit.stop="saveEditor()">
        ...
        <div class="row flex-start">
          <span class="text-shaded scnEditLabel">Fault:</span>
          <span>
          <select v-model="item.eqid">
            <option v-for="fault in faultsFilteredByLrus" v-bind:value="fault.value" >{{ fault.label }}</option>
          </select>
        </span>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      faultsFilteredByLrus () {
        let lrusToInclude = this.item.lru === 'SHOW_ALL_OPTIONS' ? this.lrusFilteredByAtas.map(lruObj => lruObj.key) : [this.item.lru]

        let filteredFaults = this.editStore.faults.filter(fault => {
          return lrusToInclude.indexOf(fault.lruid) !== -1
        })

        if (filteredFaults.length === 1) {
          this.$nextTick(function () {
            this.item.eqid = filteredFaults[0].value
          })
        }
