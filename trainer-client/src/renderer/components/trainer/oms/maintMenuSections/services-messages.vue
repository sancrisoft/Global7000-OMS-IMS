    ...
    <maintmenu-sorted-datalist v-else
      ...
      <div slot="maint-data-table-content">
        <li class="hoverable-list-item" v-for="item in sortedPage"
          ...
          <services-messages-item-display :item="item" :editMode="editMode" :title="title"></services-messages-item-display>
        </li>
      </div>
...
export default {
  ...
  methods: {
    addItem () {
      this.$bus.$emit('openScnBuilderEdit', { 'type': 'servicemessages', 'title': this.title, 'data': {}, 'action': 'add' })
    }
  }
}
</script>
